...
def audit_lp_hero():
    print(f"🕵️ AUDITING 'lp_hero' in {DB_PATH}...")
    ...
    try:
        ...
        cursor.execute("SELECT * FROM directus_collections WHERE collection = 'lp_hero'")
        row = cursor.fetchone()
        ...
        cursor.execute("SELECT field, type, interface, sort FROM directus_fields WHERE collection = 'lp_hero' ORDER BY sort")
