...
async function createRadarSection() {
    console.log("🚀 CREATING 'radar_section' (Fresh after Restart)...");

    try {
        const token = await getAuthToken();
        const client = createDirectus(DIRECTUS_URL).with(staticToken(token)).with(rest());

        // 1. CREATE COLLECTION (List Mode - Safe)
        console.log("   - Creating Collection...");
        await client.request(createCollection({
            collection: 'radar_section',
            schema: {},
            meta: {
                singleton: false,
                hidden: false,
                icon: 'radar',
                note: 'Silent Killer Radar Data',
                sort: 3
            }
        }));
        ...
        const fields = [
            ...
            { field: 'node_1_chart_type', type: 'string', meta: { interface: 'select-dropdown', width: 'half', sort: 5, options: { choices: [{ text: 'Bar', value: 'bar_chart_decline' }, { text: 'Radial', value: 'radial_progress' }, { text: 'Alert', value: 'alert_box' }] } } },
            ...
            { field: 'node_2_chart_type', type: 'string', meta: { interface: 'select-dropdown', width: 'half', sort: 10, options: { choices: [{ text: 'Bar', value: 'bar_chart_decline' }, { text: 'Radial', value: 'radial_progress' }, { text: 'Alert', value: 'alert_box' }] } } },
            ...
            { field: 'node_3_chart_type', type: 'string', meta: { interface: 'select-dropdown', width: 'half', sort: 15, options: { choices: [{ text: 'Bar', value: 'bar_chart_decline' }, { text: 'Radial', value: 'radial_progress' }, { text: 'Alert', value: 'alert_box' }] } } },
            { field: 'node_3_bullets', type: 'json', meta: { interface: 'tags', width: 'full', sort: 16 } },
        ];

        for (const f of fields) {
            await client.request(createField('radar_section', f));
        }

        // 3. SEED DATA
        console.log("   - Seeding Data...");
        await client.request(createItem('radar_section', {
        ...
        console.log("✅ 'radar_section' CREATED & SEEDED SUCCESSFULLY!");
