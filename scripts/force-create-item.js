import { createDirectus, rest, staticToken, createItem, readItems, updateCollection } from '@directus/sdk';
...
async function forceInit() {
    console.log("🔋 FORCE INITIALIZING 'lp_silent_killer' CONTENT...");

    try {
        const token = await getAuthToken();
        const client = createDirectus(DIRECTUS_URL).with(staticToken(token)).with(rest());

        // 1. REFRESH META (Toggle Singleton to wake up cache)
        console.log("   - Refreshing Collection Meta...");
        await client.request(updateCollection('lp_silent_killer', {
            meta: { singleton: false } // Toggle OFF
        }));
        await client.request(updateCollection('lp_silent_killer', {
            meta: { singleton: true } // Toggle ON
        }));
        ...
        if (items.length > 0) {
            console.log("   ✅ Rows exist:", items.length);
        } else {
            console.log("   ⚠️ No rows found. Creating Row 1...");
            ...
            console.log("   ✅ Row 1 Created Successfully!");
        }
