$ErrorActionPreference = "Stop"

$email = "admin@spead.ai"
$password = "password123"
$baseUrl = "http://localhost:8055"

Write-Host "Logging in as $email..."
$body = @{
    email    = $email
    password = $password
} | ConvertTo-Json

try {
    $response = Invoke-RestMethod -Uri "$baseUrl/auth/login" -Method Post -Body $body -ContentType "application/json"
    $token = $response.data.access_token
    Write-Host "Login successful!"

    # 1. DELETE Legacy Fields
    $fieldsToDelete = @(
        "projects_title", "projects_description", "projects_keywords", "projects_og_image",
        "journey_title", "journey_description", "journey_keywords", "journey_og_image"
    )

    foreach ($field in $fieldsToDelete) {
        Write-Host "Deleting field: $field..."
        try {
            Invoke-RestMethod -Uri "$baseUrl/fields/seo/$field" -Method Delete -Headers @{ Authorization = "Bearer $token" }
            Write-Host "✅ Deleted $field"
        }
        catch {
            $msg = $_.Exception.Message
            Write-Host "⚠️ Failed/Skipped $field: $msg"
        }
    }

    # 2. ADD New Fields for News
    # Define fields individually to avoid complex array/object nesting issues in PowerShell
    
    $news_title = @{
        field = "news_title"
        type  = "string"
        meta  = @{
            interface = "input"
            display   = "raw"
            sort      = 10
            width     = "full"
            note      = "Title for the News page"
        }
    }

    $news_description = @{
        field = "news_description"
        type  = "text"
        meta  = @{
            interface = "input-multiline"
            sort      = 11
            width     = "full"
            note      = "Meta description for the News page"
        }
    }

    $news_keywords = @{
        field = "news_keywords"
        type  = "json"
        meta  = @{
            interface = "tags"
            sort      = 12
            width     = "full"
            note      = "Keywords for the News page"
        }
    }

    $news_og_image = @{
        field = "news_og_image"
        type  = "uuid"
        meta  = @{
            interface = "file-image"
            sort      = 13
            width     = "full"
            note      = "Open Graph Image for social sharing"
        }
    }

    $fieldsToAdd = @($news_title, $news_description, $news_keywords, $news_og_image)

    foreach ($fieldSpec in $fieldsToAdd) {
        $fieldName = $fieldSpec.field
        Write-Host "Creating field: $fieldName..."
        try {
            # Depth 10 ensures nested objects (meta) are converted correctly
            $jsonBody = $fieldSpec | ConvertTo-Json -Depth 10 
            Invoke-RestMethod -Uri "$baseUrl/fields/seo" -Method Post -Body $jsonBody -ContentType "application/json" -Headers @{ Authorization = "Bearer $token" }
            Write-Host "✅ Created $fieldName"
        }
        catch {
            $errMessage = $_.Exception.Message
            if ($errMessage -match "403") {
                Write-Error "Permission Denied. Are you Admin?"
            }
            elseif ($errMessage -match "Conflict") {
                Write-Host "ℹ️ Field $fieldName might already exist."
            }
            else {
                # Try to parse deeper error if available
                if ($_.Exception.Response) {
                    $reader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
                    $respBody = $reader.ReadToEnd()
                    Write-Error "Failed to create $fieldName: $errMessage. Details: $respBody"
                }
                else {
                    Write-Error "Failed to create $fieldName: $errMessage"
                }
            }
        }
    }
    
    Write-Host "`nSEO Collection Refactoring Complete!"
}
catch {
    Write-Error "Fatal Error: $($_.Exception.Message)"
    exit 1
}
exit 0
